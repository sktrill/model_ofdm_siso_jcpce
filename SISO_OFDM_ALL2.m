...
for amplitude = 1:0.5:amp
 ...
 % Get current OFDM symbol. Remove the cyclic prefix
       ...
       PHN = exp(j*phase_noise(1,1:N));
       CFO_pre = exp(j*2*pi*freq_offset*(1:N)*sampling_period);
 % JCPCE
       F = dftmtx(s)/sqrt(s);
       ...
       %theta_est = theta_est.';
       %rx_output3 = rx_output2.*exp(-j*theta_est(1,1:N));
       
       rx_output2 = rx_output1.*exp(j*2*pi*freq_offset*(1:N)*sampling_period);
       rx_output3 = rx_output2.*exp(-j*phase_noise(1,1:N));
       
       rx_output = fft(rx_output3);
